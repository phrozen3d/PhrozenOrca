...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in Phrozen Orca before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in Phrozen Orca before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Phrozen Orca before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in Phrozen Orca before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in Phrozen Orca before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in Phrozen Orca before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Phrozen Orca before uploading."},
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on Phrozen Orca before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Phrozen Orca."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current OrcaSlicer Version %1%")%Phrozen_VERSION;
        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (is_bbl_3mf)
                {
                    ...
                    Semver cli_ver = *Semver::parse(Phrozen_VERSION);
                    if (!allow_newer_file && ((cli_ver.maj() != file_version.maj()) || (cli_ver.min() < file_version.min()))){
                        BOOST_LOG_TRIVIAL(error) << boost::format("Version Check: File Version %1% not supported by current cli version %2%")%file_version.to_string() %Phrozen_VERSION;
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            m_print_config.save_to_json(m_config.opt_string(opt_key), std::string("project_settings"), std::string("project"), std::string(Phrozen_VERSION));
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: phrozen-orca [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
