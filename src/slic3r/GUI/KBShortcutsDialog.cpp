...
void KBShortcutsDialog::OnSelectTabel(wxCommandEvent &event)
{
    ...
    while (i != m_hash_selector.end()) {
        Select *sel = i->second;
        if (id == sel->m_index) {
            sel->m_tab_button->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#EB8550"))); // ORCA color for selected tab background
            sel->m_tab_text->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#EB8550"))); // ORCA color for selected tab background
