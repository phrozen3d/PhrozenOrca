...
void KBShortcutsDialog::OnSelectTabel(wxCommandEvent &event)
{
    ...
    while (i != m_hash_selector.end()) {
        Select *sel = i->second;
        if (id == sel->m_index) {
            sel->m_tab_button->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#98593D"))); // ORCA color for selected tab background
            sel->m_tab_text->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#98593D"))); // ORCA color for selected tab background
