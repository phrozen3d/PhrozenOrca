...
/*#ifdef __WXOSX__
    ...
    StateColor box_border_colour(std::pair<wxColour, int>(wxColour("#FF7C3F"), StateColor::Normal)); // ORCA match border color with other input/combo boxes
...
void PhrozenLCDTabPrinter::init_options_list()
{
    if (!m_options_list.empty())
        m_options_list.clear();
...
void PhrozenLCDTabPrinter::msw_rescale()
{
    Tab::msw_rescale();
...
void PhrozenLCDTabSLAMaterial::init_options_list()
{
    if (!m_options_list.empty())
        m_options_list.clear();
...
void PhrozenLCDTabPrint::build()
{
    ...
    auto optgroup = page->new_optgroup(L("LCD_Layer & Exposure"), L"PhrozenImages_Resin/param_layer & Exposure");
...
void PhrozenLCDTabPrint::reload_config()
{
    this->compatible_widget_reload(m_compatible_printers);
    Tab::reload_config();
}

void PhrozenLCDTabPrint::update_description_lines()
{
...
void PhrozenLCDTabPrint::toggle_options()
{
    if (!m_active_page) return;
...
void PhrozenLCDTabPrint::update()
{
    if (m_preset_bundle->printers.get_selected_preset().printer_technology() == ptSLA)
        return; // ys_FIXME
...
void PhrozenLCDTabPrint::clear_pages()
{
    Tab::clear_pages();
...
//BBS: GUI refactor
PhrozenLCDTabPrintModel::PhrozenLCDTabPrintModel(ParamsPanel* parent, std::vector<std::string> const & keys)
    : PhrozenLCDTabPrint(parent, Preset::TYPE_MODEL)
...
void PhrozenLCDTabPrintModel::build()
{
    m_presets = &m_prints;
    PhrozenLCDTabPrint::build();
    init_options_list();
...
void PhrozenLCDTabPrintModel::set_model_config(std::map<ObjectBase *, ModelConfig *> const & object_configs)
{
...
void PhrozenLCDTabPrintModel::update_model_config()
{
    ...
    if (m_active_page)
        m_active_page->update_visibility(m_mode, true); // for taggle line
    update_dirty();
    PhrozenLCDTabPrint::reload_config();
    //update();
...
void PhrozenLCDTabPrintModel::reset_model_config()
{
...
bool PhrozenLCDTabPrintModel::has_key(std::string const& key)
{
    return std::find(m_keys.begin(), m_keys.end(), key) != m_keys.end();
}

void PhrozenLCDTabPrintModel::activate_selected_page(std::function<void()> throw_if_canceled)
{
    PhrozenLCDTabPrint::activate_selected_page(throw_if_canceled);
    if (m_active_page) {
        for (auto k : m_null_keys) {
            auto f = m_active_page->get_field(k);
            if (f)
...
void PhrozenLCDTabPrintModel::on_value_change(const std::string& opt_key, const boost::any& value)
{
    // TODO: support opt_index, translate by OptionsGroup's m_opt_map
    auto k = opt_key;
    if (m_config_manipulation.is_applying()) {
        PhrozenLCDTabPrint::on_value_change(opt_key, value);
        return;
    }
    if (!has_key(k))
        return;
    ...
    m_back_to_sys = false;
    PhrozenLCDTabPrint::on_value_change(k, value);
    for (auto config : m_object_configs) {
        config.second->touch();
        notify_changed(config.first);
    }
...
void PhrozenLCDTabPrintModel::reload_config()
{
    PhrozenLCDTabPrint::reload_config();
...
void PhrozenLCDTabPrintModel::update_custom_dirty()
{
...
//BBS: GUI refactor
PhrozenLCDTabPrintPlate::PhrozenLCDTabPrintPlate(ParamsPanel* parent) :
    PhrozenLCDTabPrintModel(parent, plate_keys)
{
...
void PhrozenLCDTabPrintPlate::build()
{
    m_presets = &m_prints;
    load_initial_data();
...
void PhrozenLCDTabPrintPlate::reset_model_config()
{
    if (m_object_configs.empty()) return;
    wxGetApp().plater()->take_snapshot(std::string("Reset Options"));
    for (auto plate_item : m_object_configs) {
...
void PhrozenLCDTabPrintPlate::on_value_change(const std::string& opt_key, const boost::any& value)
{
    auto k = opt_key;
    if (m_config_manipulation.is_applying()) {
        return;
...
void PhrozenLCDTabPrintPlate::notify_changed(ObjectBase* object)
{
...
void PhrozenLCDTabPrintPlate::update_custom_dirty()
{
    for (auto k : m_null_keys)
        m_options_list[k] = 0;
...
PhrozenLCDTabPrintObject::PhrozenLCDTabPrintObject(ParamsPanel* parent) :
    PhrozenLCDTabPrintModel(parent, concat(PrintObjectConfig().keys(), PrintRegionConfig().keys()))
{
    m_parent_tab = wxGetApp().get_tab(Preset::TYPE_PRINT);
}

void PhrozenLCDTabPrintObject::notify_changed(ObjectBase * object)
{
...
PhrozenLCDTabPrintPart::PhrozenLCDTabPrintPart(ParamsPanel* parent) :
    PhrozenLCDTabPrintModel(parent, PrintRegionConfig().keys())
{
    m_parent_tab = wxGetApp().get_model_tab();
}

void PhrozenLCDTabPrintPart::notify_changed(ObjectBase * object)
{
...
PhrozenLCDTabPrintLayer::PhrozenLCDTabPrintLayer(ParamsPanel* parent) :
    PhrozenLCDTabPrintModel(parent, concat({ layer_height }, PrintRegionConfig().keys()))
{
    m_parent_tab = wxGetApp().get_model_tab();
}

void PhrozenLCDTabPrintLayer::notify_changed(ObjectBase * object)
{
    for (auto config : m_object_configs) {
...
void PhrozenLCDTabPrintLayer::update_custom_dirty()
{
    for (auto k : m_null_keys) m_options_list[k] = 0;
    for (auto k : m_all_keys) m_options_list[k] &= ~osSystemValue;
...
const std::string& PhrozenLCDTabFilament::get_custom_gcode(const t_config_option_key& opt_key)
{
    return m_config->opt_string(opt_key, unsigned(0));
}

void PhrozenLCDTabFilament::set_custom_gcode(const t_config_option_key& opt_key, const std::string& value)
{
...
void PhrozenLCDTabFilament::add_filament_overrides_page()
{
    //BBS
...
void PhrozenLCDTabFilament::update_filament_overrides_page(const DynamicPrintConfig* printers_config)
{
...
void PhrozenLCDTabFilament::build()
{
...
// Reload current config (aka presets->edited_preset->config) into the UI fields.
void PhrozenLCDTabFilament::reload_config()
{
    this->compatible_widget_reload(m_compatible_printers);
    this->compatible_widget_reload(m_compatible_prints);
    Tab::reload_config();
...
void PhrozenLCDTabFilament::update_description_lines()
{
    Tab::update_description_lines();
...
void PhrozenLCDTabFilament::toggle_options()
{
    if (!m_active_page)
        return;
...
void PhrozenLCDTabFilament::update()
{
    if (m_preset_bundle->printers.get_selected_preset().printer_technology() == ptSLA)
        return; // ys_FIXME
...
void PhrozenLCDTabFilament::clear_pages()
{
    Tab::clear_pages();
...
void PhrozenLCDTabPrinter::build()
{
...
void PhrozenLCDTabPrinter::build_fff()
{
    if (!m_pages.empty())
        m_pages.resize(0);
...
void PhrozenLCDTabPrinter::build_sla()
{
    //if (!m_pages.empty())
    //    m_pages.resize(0);
...
void PhrozenLCDTabPrinter::extruders_count_changed(size_t extruders_count)
{
...
void PhrozenLCDTabPrinter::append_option_line(ConfigOptionsGroupShp optgroup, const std::string opt_key)
{
...
PageShp PhrozenLCDTabPrinter::build_kinematics_page()
{
 ...
 * */
void PhrozenLCDTabPrinter::build_unregular_pages(bool from_initial_build/* = false*/)
{
...
void PhrozenLCDTabPrinter::on_preset_loaded()
{
    // Orca
...
void PhrozenLCDTabPrinter::update_pages()
{
...
void PhrozenLCDTabPrinter::reload_config()
{
    Tab::reload_config();
...
void PhrozenLCDTabPrinter::activate_selected_page(std::function<void()> throw_if_canceled)
{
...
void PhrozenLCDTabPrinter::clear_pages()
{
    Tab::clear_pages();
    m_reset_to_filament_color = nullptr;
}

void PhrozenLCDTabPrinter::toggle_options()
{
...
void PhrozenLCDTabPrinter::update()
{
    m_update_cnt++;
...
void PhrozenLCDTabPrinter::update_fff()
{
...
void PhrozenLCDTabPrinter::update_sla()
{ ; }

// Return a callback to create a TabPrinter widget to edit bed shape
wxSizer* PhrozenLCDTabPrinter::create_bed_shape_widget(wxWindow* parent)
{
    // ORCA modernize button style
    Button* btn = new Button(parent, _(L("Set")) + " " + dots);
    btn->SetStyle(ButtonStyle::Regular, ButtonType::Parameter);
...
void PhrozenLCDTabPrinter::cache_extruder_cnt(const DynamicPrintConfig* config/* = nullptr*/)
{
...
bool PhrozenLCDTabPrinter::apply_extruder_cnt_from_cache()
{
...
void PhrozenLCDTabSLAMaterial::build()
{
...
// Reload current config (aka presets->edited_preset->config) into the UI fields.
void PhrozenLCDTabSLAMaterial::reload_config()
{
    this->compatible_widget_reload(m_compatible_printers);
    this->compatible_widget_reload(m_compatible_prints);
    Tab::reload_config();
...
void PhrozenLCDTabSLAMaterial::toggle_options()
{
...
void PhrozenLCDTabSLAMaterial::update()
{
...
void PhrozenLCDTabSLAPrint::build()
{
...
// Reload current config (aka presets->edited_preset->config) into the UI fields.
void PhrozenLCDTabSLAPrint::reload_config()
{
    this->compatible_widget_reload(m_compatible_printers);
    Tab::reload_config();
}

void PhrozenLCDTabSLAPrint::update_description_lines()
{
    Tab::update_description_lines();
...
void PhrozenLCDTabSLAPrint::toggle_options()
{
    if (m_active_page)
        m_config_manipulation.toggle_print_sla_options(m_config);
}

void PhrozenLCDTabSLAPrint::update()
{
    if (m_preset_bundle->printers.get_selected_preset().printer_technology() == ptFFF)
        return;
...
void PhrozenLCDTabSLAPrint::clear_pages()
{
    Tab::clear_pages();
