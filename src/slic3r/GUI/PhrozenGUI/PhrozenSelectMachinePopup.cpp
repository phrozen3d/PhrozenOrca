...
//wxDEFINE_EVENT(EVT_PHROZEN_UPDATE_WINDOWS_POSITION, wxCommandEvent);
wxDEFINE_EVENT(EVT_PHROZEN_UPDATE_USER_MACHINE_LIST, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_BIND_MACHINE, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_UNBIND_MACHINE, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_DISSMISS_MACHINE_LIST, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_CONNECT_LAN_PRINT, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_EDIT_PRINT_NAME, wxCommandEvent);
//wxDEFINE_EVENT(EVT_PHROZEN_CLEAR_IPADDRESS, wxCommandEvent);
    ...
    : PopupWindow(parent, wxBORDER_NONE | wxPU_CONTAINS_CONTROLS), m_dismiss(false)
{
    ...
    m_panel_ping_code = new PhrozenPinCodePanel(m_scrolledWindow, PinCodePanelType::BindWithPinCode, wxID_ANY, wxDefaultPosition, PHROZEN_SELECT_MACHINE_ITEM_SIZE);
    //m_panel_direct_connection = new PhrozenPinCodePanel(m_scrolledWindow, PinCodePanelType::BindWithAccessCode, wxID_ANY, wxDefaultPosition, PHROZEN_SELECT_MACHINE_ITEM_SIZE);
    m_panel_direct_connection = new PhrozenPinCodePanel(m_scrolledWindow, PinCodePanelType::BindWithIP, wxID_ANY, wxDefaultPosition, PHROZEN_SELECT_MACHINE_ITEM_SIZE);
...
PhrozenPinCodePanel::PhrozenPinCodePanel(wxWindow* parent, int type, wxWindowID winid /*= wxID_ANY*/, const wxPoint& pos /*= wxDefaultPosition*/, const wxSize& size /*= wxDefaultSize*/)
 {
     wxPanel::Create(parent, winid, pos);
     Bind(wxEVT_PAINT, &PhrozenPinCodePanel::OnPaint, this);
     SetSize(PHROZEN_SELECT_MACHINE_ITEM_SIZE);
     SetMaxSize(PHROZEN_SELECT_MACHINE_ITEM_SIZE);
     SetMinSize(PHROZEN_SELECT_MACHINE_ITEM_SIZE);

     m_type = (PhrozenPinCodePanelType)type;
     m_bitmap = ScalableBitmap(this, "bind_device_ping_code",10);
     
     this->Bind(wxEVT_ENTER_WINDOW, &PhrozenPinCodePanel::on_mouse_enter, this);
     this->Bind(wxEVT_LEAVE_WINDOW, &PhrozenPinCodePanel::on_mouse_leave, this);
     this->Bind(wxEVT_LEFT_UP, &PhrozenPinCodePanel::on_mouse_left_up, this);
 }

 void PhrozenPinCodePanel::OnPaint(wxPaintEvent& event)
 {
     wxPaintDC dc(this);
     render(dc);
 }

 void PhrozenPinCodePanel::render(wxDC& dc)
 {
#ifdef __WXMSW__
     wxSize     size = GetSize();
     wxMemoryDC memdc;
 ...
 void PhrozenPinCodePanel::doRender(wxDC& dc)
 {
     auto size = GetSize();
     ...
     wxString txt;
     if (m_type == PhrozenPinCodePanelType::BindWithPinCode )          {txt = _L("Bind with Pin Code");}
     else if (m_type == PhrozenPinCodePanelType::BindWithAccessCode )  {txt = _L("Bind with Access Code");}
     else if ( m_type == PhrozenPinCodePanelType::BindWithIP )         {txt = _L("Bind with IP");}
 ...
 void PhrozenPinCodePanel::on_mouse_enter(wxMouseEvent& evt)
 {
     m_hover = true;
     Refresh();
 }

 void PhrozenPinCodePanel::on_mouse_leave(wxMouseEvent& evt)
 {
     m_hover = false;
     Refresh();
 }

 void PhrozenPinCodePanel::on_mouse_left_up(wxMouseEvent& evt)
 {
     if (m_type == PhrozenPinCodePanelType::BindWithPinCode) {
         wxGetApp().popup_ping_bind_dialog();
     }
     else if (m_type == PhrozenPinCodePanelType::BindWithAccessCode) {
         InputIpAddressDialog dlgo;
         dlgo.ShowModal();
     }
     else if (m_type == PhrozenPinCodePanelType::BindWithIP) {
         InputIpAddressDialog dlgo;
         dlgo.SetEnablePhrozenMonitor( true );
         dlgo.ShowModal();
     }
