...
Sidebar::Sidebar(Plater *parent)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(lround(43.5 * wxGetApp().em_unit()), -1)), p(new priv(parent))
{
...
{
    SetMinSize(wxSize(lround(43.5 * wxGetApp().em_unit()), -1));
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update Phrozen Orca or restart Phrozen Orca to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    ...
    , partplate_list(this->q, &model)
{
    ...
    m_aui_mgr.AddPane(sidebar, wxAuiPaneInfo()
                                   ...
                                   .Floatable(true)
                                   .BestSize(wxSize(lround(43.5 * wxGetApp().em_unit()), 90 * wxGetApp().em_unit())));
        ...
        try {
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    // BBS: version check
                    Semver app_version = *(Semver::parse(Phrozen_VERSION));
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        // do not reset the model config
                        load_config = false;
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by PhrozenOrca, load geometry data only."), _L("Load 3mf"));
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Phrozen Orca, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by PhrozenOrca, load geometry data only."), _L("Load 3mf"));
                    //     }
                    ...
                    // Orca: check if the project is created with OrcaSlicer 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like PhrozenOrca to automatically fix this by clearing the rotation template settings?"));
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - PhrozenOrca");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Phrozen Orca window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Phrozen Orca failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    , m_action(2)
{
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/PhrozenOrcaTitle.ico") % resources_dir()).str();
...
void Plater::export_stl(bool extended, bool selection_only, bool multi_stls)
{
    ...
    auto mesh_to_export_sla = [&, this](const ModelObject& mo, int instance_id) {
        ...
        if (auto m = object->get_mesh_to_slice(); m.empty())
...
int Plater::export_3mf(const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx, Export3mfProgressFn proFn)
{
    ...
    // SLA: sync auto-generated support points from backend to ModelObject before saving.
    // When sla_points_status == AutoGenerated, the points only exist in po->m_supportdata->pts
    // (backend) and m_normal_cache (gizmo frontend), NOT in mo->sla_support_points.
    // We must copy them here so bbs_3mf can write them to Slic3r_PE_sla_support_points.txt.
    if (p->printer_technology == ptSLA) {
        const SLAPrint *sla_print = p->background_process.sla_print();
        if (sla_print) {
            for (const SLAPrintObject *po : sla_print->objects()) {
                ModelObject *mo = const_cast<ModelObject*>(po->model_object());
                if (!mo) continue;
                if (mo->sla_points_status == sla::PointsStatus::AutoGenerated) {
                    // Copy backend points (in print object space) back to model object space.
                    const std::vector<sla::SupportPoint> &pts = po->get_support_points();
                    if (!pts.empty()) {
                        auto mat = po->trafo().inverse().cast<float>();
                        mo->sla_support_points.clear();
                        mo->sla_support_points.reserve(pts.size());
                        for (const sla::SupportPoint &sp : pts)
                            mo->sla_support_points.emplace_back(mat * sp.pos, sp.head_front_radius, sp.type);
                    }
                }
            }
        }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Phrozen Orca(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
