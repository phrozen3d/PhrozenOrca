...
public:
    ...
    void Decorate(wxBitmap& bmp)
    {
        ...
        auto bs_version = wxString::Format(_L("Based on OrcaSlicer, PrusaSlicer and BambuStudio")).ToStdString();
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("PhrozenOrca will terminate because of running out of memory. "
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("PhrozenOrca will terminate because of a localization error. "
                             "It will be appreciated if you report the specific scenario this issue happened.");
        wxMessageBox(errmsg + "\n\n" + wxString(ex.what()), _L("Critical error"), wxOK | wxICON_ERROR);
        std::terminate();
        //throw;
    } catch (const std::exception& ex) {
        wxLogError(format_wxstr(_L("PhrozenOrca got an unhandled exception: %1%"), ex.what()));
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        throw;
    }
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("PhrozenOrca ");
    if (start == std::string::npos) {
        start = body.find("PhrozenOrca ");
        if (start == std::string::npos)
            return boost::none;
    }
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(_L("Phrozen Orca requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "PhrozenOrca", wxICON_QUESTION | wxYES_NO);
    ...
    if (is_editor()) {
            ...
            if (this->plater_ != nullptr) {
               ...
               /* wxString tips = wxString::Format(_L("Click to download new version in default browser: %s"), version_info.version_str);
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("New version of Phrozen Orca"),
                    false,
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("The Phrozen Orca needs an upgrade"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The PhrozenOrca configuration file may be corrupted and cannot be parsed.\nPhrozenOrca has attempted to recreate the "
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #FF7C3F  -- 73
        if (bg_col != orig_col) {
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of Phrozen Orca is too low and needs to be updated to the latest version before it can be used normally."), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
...
{
	wxArrayString translations = wxTranslations::Get()->GetAvailableTranslations(L10N_CATALOG_DOMAIN);
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by PhrozenOrca.conf: %1%") % language;
        else {
            ...
            if (lang_system != wxLANGUAGE_UNKNOWN) {
            ...
            } else {
                {
                    ...
                    wxString best_language = wxTranslations::Get()->GetBestTranslation(L10N_CATALOG_DOMAIN, wxLANGUAGE_ENGLISH);
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by PhrozenOrca: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching Phrozen Orca to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "Phrozen Orca - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
    ...
    m_wxLocale->AddCatalog(L10N_CATALOG_DOMAIN);
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto 3 icon in Finder,
