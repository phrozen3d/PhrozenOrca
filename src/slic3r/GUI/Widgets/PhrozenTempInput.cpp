...
EVT_ENTER_WINDOW(PhrozenTempInput::mouseEnterWindow)
EVT_LEAVE_WINDOW(PhrozenTempInput::mouseLeaveWindow)
EVT_KEY_DOWN(PhrozenTempInput::keyPressed)
EVT_KEY_UP(PhrozenTempInput::keyReleased)
EVT_MOUSEWHEEL(PhrozenTempInput::mouseWheelMoved)
EVT_PAINT(PhrozenTempInput::paintEvent)
END_EVENT_TABLE()


PhrozenTempInput::PhrozenTempInput()
    ...
    , text_color(std::make_pair(wxColour(0xAC,0xAC,0xAC), (int) StateColor::Disabled), std::make_pair(0x6B6B6B, (int) StateColor::Normal))
{
    hover  = false;
    radius = 0;
    border_color = StateColor(std::make_pair(*wxWHITE, (int) StateColor::Disabled), std::make_pair(0xFF7C3F, (int) StateColor::Focused), std::make_pair(0xFF7C3F, (int) StateColor::Hovered),
...
PhrozenTempInput::PhrozenTempInput(wxWindow *parent, int type, wxString text, wxString label, wxString normal_icon, wxString actice_icon, const wxPoint &pos, const wxSize &size, long style)
    : PhrozenTempInput()
{
    actice = false;
    temp_type = type;
...
void PhrozenTempInput::Create(wxWindow *parent, wxString text, wxString label, wxString normal_icon, wxString actice_icon, const wxPoint &pos, const wxSize &size, long style)
{
...
bool PhrozenTempInput::AllisNum(std::string str)
{
...
wxString PhrozenTempInput::erasePending(wxString &str)
{
    wxString tmp   = str;
...
void PhrozenTempInput::SetTagTemp(int temp)
{
...
void PhrozenTempInput::SetTagTemp(wxString temp)
{
...
void PhrozenTempInput::SetCurrTemp(int temp)
{
    SetLabel(wxString::Format("%d", temp));
}

void PhrozenTempInput::SetCurrTemp(wxString temp)
{
    SetLabel(temp);
}

void PhrozenTempInput::Warning(bool warn, WarningType type)
{
    warning_mode = warn;
    //Refresh();
...
void PhrozenTempInput::SetIconActive()
{
    actice = true;
    Refresh();
}

void PhrozenTempInput::SetIconNormal()
{
    actice = false;
    Refresh();
}

void PhrozenTempInput::SetMaxTemp(int temp) { max_temp = temp; }

void PhrozenTempInput::SetMinTemp(int temp) { min_temp = temp; }

void PhrozenTempInput::SetLabel(const wxString &label)
{
    wxWindow::SetLabel(label);
    messureSize();
    Refresh();
...
void PhrozenTempInput::SetTextColor(StateColor const &color)
{
    text_color = color;
    state_handler.update_binds();
}

void PhrozenTempInput::SetLabelColor(StateColor const &color)
{
    label_color = color;
    state_handler.update_binds();
}

void PhrozenTempInput::Rescale()
{
...
bool PhrozenTempInput::Enable(bool enable)
{
...
void PhrozenTempInput::SetMinSize(const wxSize &size)
{
    wxSize size2 = size;
    if (size2.y < 0) {
#ifdef __WXMAC__
...
void PhrozenTempInput::DoSetSize(int x, int y, int width, int height, int sizeFlags)
{
...
void PhrozenTempInput::DoSetToolTipText(wxString const &tip)
{
    wxWindow::DoSetToolTipText(tip);
    text_ctrl->SetToolTip(tip);
}

void PhrozenTempInput::paintEvent(wxPaintEvent &evt)
{
 ...
 */
void PhrozenTempInput::render(wxDC &dc)
{
    StaticBox::render(dc);
    ...
    if (warning_mode) {
        ...
        border_color = StateColor(std::make_pair(*wxWHITE, (int) StateColor::Disabled), std::make_pair(0xFF7C3F, (int) StateColor::Focused),
                                  std::make_pair(0xFF7C3F, (int) StateColor::Hovered), std::make_pair(*wxWHITE, (int) StateColor::Normal));
    }
...
void PhrozenTempInput::messureMiniSize()
{
...
void PhrozenTempInput::messureSize()
{
    wxSize size = GetSize();
...
void PhrozenTempInput::mouseEnterWindow(wxMouseEvent &event)
{
    if (!hover) {
        hover = true;
        Refresh();
...
void PhrozenTempInput::mouseLeaveWindow(wxMouseEvent &event)
{
    if (hover) {
        hover = false;
        Refresh();
...
void PhrozenTempInput::mouseMoved(wxMouseEvent &event) {}
void PhrozenTempInput::mouseWheelMoved(wxMouseEvent &event) {}
void PhrozenTempInput::keyPressed(wxKeyEvent &event) {}
void PhrozenTempInput::keyReleased(wxKeyEvent &event) {}
