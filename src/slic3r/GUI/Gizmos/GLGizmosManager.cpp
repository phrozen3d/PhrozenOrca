...
void GLGizmosManager::update_after_undo_redo(const UndoRedo::Snapshot& snapshot)
{
    ...
    // Decision K (openspec/changes/resin-mode-scoped-undo-redo/design.md): resync every resin
    // gizmo unconditionally on every undo/redo landing on a valid, SLA-technology object —
    // not gated on which gizmo (if any) is currently active (m_current; missed redo landing on
...
bool GLGizmosManager::resolve_editing_mode_before_close()
{
    if (m_current != SlaSupports)
        return true;
    auto *gizmo = dynamic_cast<GLGizmoSlaSupports *>(get_current());
    return gizmo ? gizmo->resolve_editing_mode_before_close() : true;
}
