...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (int extruder_idx = 0; extruder_idx < n_extruder_colors; extruder_idx++) {
        ...
        #ifdef __APPLE__
            ImGui::PushStyleColor(ImGuiCol_FrameBg, ImGuiWrapper::COL_PHROZEN); // ORCA use orca color for selected filament border
            ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 0.0f);
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (m_current_tool == tool_ids[i]) {
            ImGui::PushStyleColor(ImGuiCol_Button,          ImVec4(255 / 255.f, 176 / 255.f, 140 / 255.f, 0.25f));  // Phrozen use phrozen color for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered,   ImVec4(255 / 255.f, 176 / 255.f, 140 / 255.f, 0.25f));  // Phrozen use phrozen for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_ButtonActive,    ImVec4(255 / 255.f, 176 / 255.f, 140 / 255.f, 0.30f));  // Phrozen use phrozen for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_Border,          ImGuiWrapper::COL_PHROZEN);            // Phrozen use orca Phrozen for border on selected tool / brush
