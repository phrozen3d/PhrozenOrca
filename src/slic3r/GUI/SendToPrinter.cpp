    ...
    , m_plater(plater), m_export_3mf_cancel(false)
{
    ...
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(0, 137, 123), StateColor::Pressed), std::pair<wxColour, int>(wxColour(240, 94, 32), StateColor::Hovered),
...
void SendToPrinterDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
	...
	if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text = _L("The printer is required to be in the same LAN as Phrozen Orca.");
        update_print_status_msg(msg_text, true, true);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
    }
