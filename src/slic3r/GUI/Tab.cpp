...
void TabPrint::build()
{
    ...
    optgroup->append_single_option_line("initial_layer_speed", "speed_settings_initial_layer_speed#initial-layer");
        ...
        line.append_option(optgroup->get_option("overhang_0_4_speed"));
...
PageShp TabPrinter::build_kinematics_page()
{
    ...
    {
        Line resonance_line = {L("Resonance Avoidance Speed"), ""};
...
bool Tab::select_preset(std::string preset_name, bool delete_current /*=false*/, const std::string& last_selected_ph_printer_name/* =""*/, bool force_select)
{
    ...
    if (canceled) {
        ...
        if (! is_selected && printer_tab)
        {
             ...
             */
            const PrinterTechnology printer_technology = m_presets->get_edited_preset().printer_technology();
            const bool dependent_tabs_empty = m_dependent_tabs.empty();
            if (printer_technology == ptFFF && (dependent_tabs_empty || m_dependent_tabs.front() != Preset::Type::TYPE_PRINT))
                m_dependent_tabs = { Preset::Type::TYPE_PRINT, Preset::Type::TYPE_FILAMENT };
            else if (printer_technology == ptSLA && (dependent_tabs_empty || m_dependent_tabs.front() != Preset::Type::TYPE_SLA_PRINT))
