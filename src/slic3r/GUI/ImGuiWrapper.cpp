...
bool ImGuiWrapper::bbl_slider_float_style(const std::string &label, float *v, float v_min, float v_max, const char *format, float power, bool clamp, const wxString &tooltip)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));
...
bool ImGuiWrapper::bbl_checkbox(const wxString &label, bool &value)
{
    ...
    bool b_value = value;
    if (b_value) {
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));
    }
    auto label_utf8 = into_u8(label);
...
void ImGuiWrapper::push_common_window_style(const float scale) {
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Separator, ImGuiWrapper::COL_SEPARATOR_DARK);                             // 5
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(1.00f, 1.00f, 1.00f, 1.00f));                              // 6
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));                       // 7
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));                        // 8
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(84 / 255.0f, 84 / 255.0f, 90 / 255.0f, 1.00f));    // 9
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(62 / 255.0f, 62 / 255.0f, 69 / 255.0f, 1.00f));     // 10
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(238 / 255.0f, 238 / 255.0f, 238 / 255.0f, 0.00f));        // 11
        ...
        ImGui::PushStyleColor(ImGuiCol_PlotHistogram, ImVec4(1.0f, 124.0f / 255.0f, 63.0f / 255.0f, 1.0f));                       // 14
    }
    else {
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * scale);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(20.0f, 10.0f) * scale);
...
void ImGuiWrapper::push_confirm_button_style() {
    const ImVec4 confirm_bg     = COL_ORANGE_LIGHT;
    const ImVec4 confirm_hover  = to_ImVec4(decode_color_to_float_array("#F05E20"));
    const ImVec4 confirm_active = confirm_hover;
    const ImVec4 confirm_text   = m_is_dark_mode ? ImVec4(1.f, 1.f, 1.f, 0.88f) : ImVec4(1.f, 1.f, 1.f, 1.f);

    ImGui::PushStyleColor(ImGuiCol_Button, confirm_bg);
    ImGui::PushStyleColor(ImGuiCol_Border, confirm_bg);
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, confirm_hover);
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, confirm_active);
    ImGui::PushStyleColor(ImGuiCol_CheckMark, confirm_text);
    ImGui::PushStyleColor(ImGuiCol_Text, confirm_text);
}
...
{
    if (m_is_dark_mode) {
        ImGui::PushStyleColor(ImGuiCol_CheckMark, to_ImVec4(decode_color_to_float_array("#FF7C3F"))); // ORCA use orca color for radio buttons
    } else {
        ImGui::PushStyleColor(ImGuiCol_CheckMark, to_ImVec4(decode_color_to_float_array("#FF7C3F"))); // ORCA use orca color for radio buttons
    }
}
