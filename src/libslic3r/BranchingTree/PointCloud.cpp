...
// Step 3.2: SupportTreeUtils.hpp is not available in PhrozenOrca.
// find_merge_pt is implemented inline below (identical to PrusaSlicer's sla::find_merge_pt).
...
// Step 3.2: find_merge_pt implementation — geometry only, no SupportTreeUtils.hpp dependency.
// Identical to PrusaSlicer's sla::find_merge_pt (SupportTreeUtils.hpp line 875).
// The idea: A and B have support cones; we reduce to 2D (vertical slice) and
// find where two rays emanating from A and B (downward at critical_angle) intersect.
                                   ...
                                   float        critical_angle)
{
    ...
    float sincrit  = std::sin(critical_angle);
    float sin2crit = -sincrit * sincrit;
    sin2sig_a = std::min(sin2sig_a, sin2crit);
    sin2sig_b = std::min(sin2sig_b, sin2crit);
    float sin2_a = std::abs(sin2sig_a);
