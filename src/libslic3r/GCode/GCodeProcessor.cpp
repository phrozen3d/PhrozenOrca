...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::PhrozenOrca, SLIC3R_APP_NAME },
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        if (m_producer == EProducer::OrcaSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r || m_producer == EProducer::PhrozenOrca) {
            ...
            if ( m_producer != EProducer::PhrozenOrca )
            {
                config.load_from_gcode_file(filename, ForwardCompatibilitySubstitutionRule::EnableSilent, "OrcaSlicer");
            }
            else
            {
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::OrcaSlicer || m_producer == EProducer::PhrozenOrca ) {
        // height tag
