...
void SLAPrint::Steps::rasterize()
{
    ...
    if (!canceled()) {
        ...
        // ---- Disk-cache rasterization pipeline --------------------------------
        // Task 5.1: compute cache key from raster params + layer ExPolygons
        const size_t N = m_print->m_printer_input.size();
        if (N == 0) {
            m_print->m_raster_cache_key = "";
            return;
        }

        const auto cache_key = sla::RasterCache::compute_key(rp, m_print->m_printer_input);

        // Task 5.2: early return if cache is already valid
        // (e.g. only exposure time changed — geometry unchanged)
        ...
        // Task 5.3: clean up old cache entries before writing new ones
        sla::RasterCache::cleanup_old();

        // Task 5.4: Pipeline synchronization primitives
        constexpr size_t BATCH_SZ    = 8;
        constexpr size_t QUEUE_DEPTH = 2;
        using Batch = std::vector<cv::Mat>;
        std::queue<Batch>       batch_queue;
        std::mutex              queue_mutex;
        std::condition_variable queue_cv_not_full, queue_cv_not_empty;
        bool                    producer_done      = false;
        std::exception_ptr      producer_exception = nullptr;
        std::atomic<bool>       cancelled{false};
        // Task 9.1: tracks how many layers the Producer has fully rasterized;
        // lets the Consumer report smooth progress while blocked waiting for batches.
        std::atomic<size_t>     producer_rasterized{0};

        // Task 5.5: Producer lambda
        auto producer_lambda = [&]() {
            try {
                for (size_t batch_start = 0; batch_start < N; batch_start += BATCH_SZ) {
                    ...
                    // [A] Collect ExPolygons with bed→display shift
                    std::vector<ExPolygons> batch_polys(batch_n);
                    for (size_t i = 0; i < batch_n; ++i) {
                        batch_polys[i] = m_print->m_printer_input[batch_start + i].transformed_slices();
                        for (ExPolygon &ep : batch_polys[i])
                            ep.translate(rp.shift);
                    }

                    // [B] TBB parallel rasterize
                            ...
                            tbb::parallel_for(tbb::blocked_range<size_t>(0, batch_n),
                                [&](const tbb::blocked_range<size_t> &r) {
                                    for (size_t i = r.begin(); i < r.end(); ++i) {
                                        ...
                                        // Task 9.2: per-layer update so Consumer sees progress
                                        // even while the slowest (solid-base) batch is still running.
                    ...
                    // [C] Push batch; block until Consumer has a free slot
                    {
        ...
        // Task 5.6: Consumer loop — output to disk cache
        ...
        try {
            while (true) {
                ...
                {
                    std::unique_lock<std::mutex> lk(queue_mutex);
                    // Task 9.3: wait with timeout so we can report Producer's progress
                    // smoothly while waiting, eliminating the ~55%→100% jump.
                    ...
                    if (batch_queue.empty()) break; // producer done, queue empty
                ...
                for (size_t i = 0; i < batch_n; ++i) {
                    ...
                    // [E] Write RLE bytes to disk cache (no length prefix, no CRLF)
                    sla::RasterCache::write_layer(cache_key, lid, przByte);
                    batch[i].release();
                }

                batch_start_consumer = batch_end;

                // [F] Progress + cancellation check
        ...
        if (consumer_rethrow)
            std::rethrow_exception(consumer_rethrow);

        // Task 5.7: Pipeline complete — finalize cache and store key
        sla::RasterCache::finalize(cache_key, N, rp);
        m_print->m_raster_cache_key = cache_key.hash;
        // ---- End disk-cache rasterization pipeline ----------------------------
    }
}
