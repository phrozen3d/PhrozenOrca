...
//		}
		...
		if(bFind)
		{
		    ...
		    {
				/* ORCA use label tag to allow checkbox to toggle when user ckicked to text; type label uses tid for translation (e.g. t_standard, t_engineering) */
			    let tid = 't_' + LowType;
			    let HtmlType='<label><input type="checkbox" filatype="'+fType+'" onChange="FilaClick()" /><span class="trans" tid="'+tid+'">'+fType+'</span></label>';
...
//				$("#ItemBlockArea input[vendor='"+fVendor+"'][model='"+fModel+"'][filatype='"+fType+"'][name='"+key+"']").prop("checked",false);			
	...
	// Translate dynamically added type labels (e.g. Standard -> 模型, Engineering -> 工程)
	TranslatePage();
...
function ResponseFilamentResult()
{
	...
	var tSend={};
	tSend['sequence_id']=Math.round(new Date() / 1000);
	// LCD Resin page: save selections as resin (SECTION_MATERIALS), not filament
	tSend['command']="save_userguide_resins";
	tSend['data']={};
	tSend['data']['resin']=FilaArray;
