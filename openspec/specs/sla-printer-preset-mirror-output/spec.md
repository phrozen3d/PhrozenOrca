# Spec: sla-printer-preset-mirror-output

## Purpose

Define how `SLAPrinterSettingsDialog` exposes and persists three named SLA mirror modes (Normal, LCD_mirror, DLP_normal), ensure that mode identity round-trips through a dedicated `display_mirror_mode` config key, and guarantee that `.prz` Xmirror/Ymirror bytes reflect the correct final raster behavior regardless of printer orientation. Also specifies the required mirror config values for the three active Phrozen SLA machine presets and the serialization format constraints for `coBool` preset fields.
