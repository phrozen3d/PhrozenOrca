# Spec: SLA 預覽降採樣 Thumb 快取

## Purpose

定義 SLA 切層 2D 預覽的「降採樣 thumb 快取」:於 rasterize 平行管線中順手產生符合 GL texture 上限的小圖並落地，UI 預覽改為只讀此快取、永不在 render thread 觸發 AGG 或全解析度解碼，從根本消除滑動預覽卡頓;並規範其格式、方位、存活性綁定、cache key 節流與顯示方位動態補償。
