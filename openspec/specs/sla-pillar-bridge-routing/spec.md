# Spec: SLA Pillar Bridge Routing

## Purpose

Defines how the SLA support tree routes bridge connections between pillars in `cascadefn`. The goal is to ensure bridges are attempted in nearest-first order and that cross-weight bridging is allowed — physical feasibility is delegated entirely to `interconnect()`.
