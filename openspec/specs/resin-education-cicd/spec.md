# Spec: resin-education-cicd

## Purpose

定義 resin/education 變體（`PHROZEN_ORCA_ENABLE_RESIN=ON`）的 CI/CD 建置管線 —— 觸發方式（僅手動）、必須產出的成品（Windows NSIS 安裝檔／portable ZIP／PDB、macOS 已簽章公證的 DMG）、artifact 命名必須與主線可區分，以及「主線共用 CI 檔案零修改」與「主線建置行為完全不受影響」這兩個結構性約束。`resin-education-variant-branding` 涵蓋的是建置產物的識別字串，本 spec 只是消費其結果，不改變其任何需求。
...
> **驗證狀態註記（2026-08-04）**：CI 管線本身（codesign／notarytool submit／stapler staple 三個步驟）已在 GitHub Actions 上實測全數執行成功。但終端使用者體感驗證（`spctl -a -vv` 顯示 `accepted`、真實 Mac 上開啟不跳 Gatekeeper 阻擋視窗等）仍待實際 Mac 硬體驗證，尚未完成。
...
> **驗證狀態註記（2026-08-04）**：此為結構性保證（主線分支缺少 `build_resin_release_vs2022.bat` / `build_resin_release_macos.sh`，對應 job 步驟必定找不到檔案而失敗），但尚未實際在 Actions UI 上選擇主線分支觸發一次來實測確認。
