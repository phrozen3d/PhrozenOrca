# Spec: sla-csg-pipeline
## PhrozenOrca SLA — CSG Pipeline 基礎架構
...
SLA print pipeline 以 `slaposAssembly` 為第一步，將模型所有 volumes 轉換為
`csg::CSGPart` 並存入 `SLAPrintObject::m_mesh_to_slice`（`std::multimap<SLAPrintObjectStep, csg::CSGPart>`）。
後續步驟（Hollowing、DrillHoles）只需清除並重新注冊自身 step 的 parts，不影響其他步驟。
