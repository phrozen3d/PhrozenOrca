...
定義 `generate_prz()` 的 PRZ 檔案匯出：依 `RasterCache::is_valid()` 走快取命中（串流現成 `.rle` bytes）或快取未命中（on-demand 雙軌光柵化 + 融合條帶編碼）兩條路徑，背景執行緒分批串流寫檔，記憶體峰值不隨層數成長，且 header 欄位映射符合 Phrozen 韌體與 Chitubox 對齊規範。
## Requirements
...
**快取未命中路徑**：以批次大小 `BATCH_SZ = 8` 迭代 `print.print_layers()`，TBB 平行對每層執行**雙軌光柵化**——取 model-track 與 support-track 幾何（各平移 `rp.shift`），依 `lid < bottom_layer_count` 決定 model 是否走二值，support 恆走二值，model 套 `apply_picture_grayscale_lut` 後與 support 合成（`output = max(model_after_LUT, support_255)`）。support 合成 SHALL 走 **support 局部 ROI**（`composite_support_binary`），**不**配置全幀 support 緩衝；`enumerable_thread_specific<cv::Mat> support_tls` SHALL 移除。此雙軌合成 SHALL 呼叫與 `rasterize()` 主迴圈相同的共用函式，確保 byte 一致；其後 SHALL 呼叫共用之 **`prz_encode_layer_banded(panel_mat, prz_x_mirror, band_cols, out)`** 一次完成「條帶化旋轉（`cv::rotate(90°CW)`）+ 局部 `prz_orient_after_rotate` + PRZ-RLE 編碼」後串流輸出。此路徑 SHALL **不**配置全幀旋轉副本（per-layer `cv::Mat rotated` SHALL 移除）；`cv::Mat` 於編碼後立即釋放。

ROI 合成與融合條帶編碼輸出 SHALL 與舊全幀路徑逐位元組相同，`CACHE_VERSION` 不變，既有快取續用。
...
#### Scenario: cache-miss 路徑不持有全幀旋轉副本
- **WHEN** 快取未命中路徑並行編碼各層
- **THEN** 不存在 per-layer 全幀旋轉 `cv::Mat rotated`；旋轉+翻轉+RLE 由 `prz_encode_layer_banded()` 以 `≤ band_cols × M` 的條帶 tile 完成

#### Scenario: 融合條帶編碼與全幀版逐 byte 一致（不 bump 版本）
- **WHEN** 同一份切片分別以融合條帶路徑與舊全幀 rotate→flip→線性 RLE 匯出 PRZ
- **THEN** 兩者每層 RLE bytes 完全相同；`CACHE_VERSION` 未變，舊路徑建立的 disk cache 在本變更後仍 `is_valid()` 命中且與 cache-miss 輸出一致
