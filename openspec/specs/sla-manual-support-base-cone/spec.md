# Spec: sla-manual-support-base-cone

## Purpose

確保每個手動放置的 SLA 支撐點（`SupportPointType::manual_add`）在 clustering 階段保持獨立，各自生成完整的地面柱（Pillar）與底座圓錐（Pedestal），同時不影響 auto-generated 支撐點的既有 clustering 行為。
