# Spec: sla-slice-model-csg
## PhrozenOrca SLA — slice_model() 改用 slice_csgmesh_ex
...
原 `slice_model()` 以 `slice_mesh_ex(hollow_mesh_with_holes)` 切片，
再額外對 hollow interior slices 執行 `diff_ex()` 處理內腔。

改為以 `slice_csgmesh_ex(range(po.m_mesh_to_slice), slicegrid, params)` 一次處理
所有 CSG parts（shell Union + hollow Difference + drain holes Difference），
2D polygon 運算在 `slice_csgmesh_ex` 內部完成，不需後置 `diff_ex` 步驟。

**Hollow interior 法向量注意**：
`generate_interior()` 產生的 mesh 法向量朝內（toward solid），
存入 CSGType::Difference 前須先 `swap_normals()`，
確保 `slice_csgmesh_ex` 對 Difference part 取得正確的實心截面後執行 `diff_ex`。
