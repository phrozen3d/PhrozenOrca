...
以下三項為本設計所依賴的假設。**三項皆已於實作階段完成驗證**（A1 / A2 於 2026-08-26 由自動測試驗證，A3 於 2026-08-27 由 GUI 手動驗收驗證），各項的實測結果記於該項末尾。原則不變：任何一項若日後被推翻，都需回到本文件重新決議，而非就地判斷。
...
**實測結果（2026-08-27，`build-resin-dbginfo` 之 `PhrozenOrca` GUI 手動驗收）**：對同一薄板模型分別施加 (a) X 軸鏡像、(b) 非等比縮放 (1.0, 1.0, 3.0)、(c) 兩者併用，三組皆於 Points 檢視目視確認**預覽支撐頭未穿透模型**，**A3 成立**，備案（改由 `po->trafo()` 導出 `head.dir`）無須啟動。

實作採用的變換為 `trafo.linear().inverse().transpose()`，即 co-vector 的 `(M^-1)^T`；純鏡像 `M = diag(1,1,-1)` 時 `(M^-1)^T = M`。鏡像會同時翻轉三角形纏繞方向，而 `po->get_mesh_to_print()` 是否處理該問題**未經直接確認**；實測顯示三組皆未穿透，即實際行為上並未因此出錯。

**驗證範圍的限制（誠實標示）**：本次驗證為**目視**確認「未穿透」。原訂驗證方式另含「比對 GUI 量得的厚度與切片端量得的厚度，在軸向一致的垂直面上兩者應相等」，**這一項未以數值逐點比對**（需於兩端同時加入日誌輸出）。目視無穿透是 A3 成立的必要條件，不是與切片端數值相等的充分條件。對應的 spec Scenario「鏡像與非等比縮放併用 → 兩者 MUST 相等」已於 `tasks.md` 4.21 記為**已知限制**。
...
**實作後的實際 commit 結構（2026-08-27 更新，以 `git merge-tree` 非破壞性模擬驗證）**

四項修改實際落成**五個** commit——GUI 因快取重構（`HeadGeomKey` 移除 `penetration`、canonical 網格、model matrix 平移）在第一次建置驗證通過之後才實作，而拆成兩筆：

| 項 | commit | 單獨 revert 的結果 |
|---|---|---|
| #6 taildir | `258c6f8b7` | **於 `SupportTreeBuildsteps.cpp` 產生真實衝突** |
| #1 方向性吸附 | `acda10a95` | 原始碼零衝突 |
| #5 切片端夾限 | `b3105e15f` | 原始碼零衝突（僅 `tasks.md` 撞） |
| GUI（量測與夾限） | `7fc68839e` | 原始碼零衝突（僅 `tasks.md` 撞） |
| GUI（快取重構） | `1cfaf103b` | 零衝突（位於分支頂端） |

**#6 的衝突是本策略的預期結果，不是缺陷。** 衝突點在 `connect_to_model_body()` 的 `taildir` / `dist` 區塊，被 #5 的夾限提交點 2 覆寫。上文已載明 #5 依賴 #6（射線需單位向量），故「保留 #5 而單獨回退 #6」本就不是有效的回退路徑。

**有效的回退路徑（依相依序，由後往前）**：

```
GUI  ->  git revert 1cfaf103b 7fc68839e     # 兩筆必須一起，且順序不可顛倒
#5   ->  git revert b3105e15f               # 需先退掉 GUI
#6   ->  git revert 258c6f8b7               # 需先退掉 #5
#1   ->  git revert acda10a95               # 獨立，任何時候皆可
```

**維護者請注意**：回退 GUI 時漏掉 `1cfaf103b` 會留下「canonical 網格 + model matrix 平移」而失去其所服務的夾限，畫面上支撐頭會停在錯誤深度。兩筆必須成對處理。
