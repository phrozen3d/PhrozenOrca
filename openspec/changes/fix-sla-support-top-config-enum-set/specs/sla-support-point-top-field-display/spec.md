# sla-support-point-top-field-display

## Overview

在 SLA Support gizmo 的手動編輯模式中選取一顆支撐點時，Process → Support → **Top** 群組的欄位應顯示**該點**的 per-point 參數，而非 SLA print preset 的值；編輯這些欄位時應寫回選中點並重畫 3D 視圖。

涉及的欄位共七個：`support_contact_type`、`support_contact_diameter`、`support_head_penetration`、`support_head_front_diameter`、`support_head_back_diameter`、`support_pillar_diameter`、`support_segment_length`。

投影邏輯位於 `support_top_apply_point(sp, cfg)`，以 `sla_process_config()` 的副本為起點逐一覆寫；`support_top_config_from_selection()` 為其唯一呼叫者。
