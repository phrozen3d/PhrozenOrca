# Spec: PRZ Band/Tiled 融合旋轉與 RLE 編碼

## Purpose

定義融合自由函式 `prz_encode_layer_banded(portrait, final_x_mirror, band_cols, out)`：以 L2/L3-friendly 直向 column-slab 條帶逐塊旋轉（`cv::rotate(ROTATE_90_CW)`）、局部翻轉（`prz_orient_after_rotate`）並就地 RLE 編碼，**永不物化全幀旋轉副本**（`mat_rotated`），且輸出與舊「全幀 rotate→flip→線性掃描」路徑逐位元組相同。
