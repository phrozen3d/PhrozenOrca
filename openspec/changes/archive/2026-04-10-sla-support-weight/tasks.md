...
> ⏸ **暫緩原因**：後續支撐演算法將大幅修改，weight 欄位的語意可能隨之調整，待演算法穩定後再實作序列化，避免格式版本頻繁升級。
> **分析文件**：proposal.md 已記錄完整的改法與風險。
...
- [x] 5.1 編譯確認零錯誤
- [x] 5.2 放置 Light 點並生成支撐，確認柱子明顯細於 Medium（含整根柱子，非僅接觸頭）
- [x] 5.3 放置 Heavy 點並生成支撐，確認柱子明顯粗於 Medium
- [x] 5.4 自動生成的 island/slope 點使用全域粗細（weight 欄位被忽略）
- [ ] 5.5 儲存 3mf 並重新載入，確認 weight 正確還原（待 Step 3 完成後驗證）
- [ ] 5.6 載入舊 3mf（5 欄位）確認預設為 Medium，程式不崩潰（待 Step 3 完成後驗證）
- [x] 5.7 確認 FDM 切片不受影響
