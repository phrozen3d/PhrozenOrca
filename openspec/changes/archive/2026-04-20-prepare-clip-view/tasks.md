...
- [x] 在 `GLCanvas3D.cpp` 新增私有方法 `_update_prepare_scene_max_z()`：
...
- [x] 在適當時機呼叫（物件增刪時、每次 render 前或 `_refresh_if_shown_on_screen()`）

### 1.3 新增 Slider 渲染方法（GLCanvas3D）

- [x] 在 `GLCanvas3D.hpp` 宣告 `void _render_prepare_clip_slider()`
- [x] 在 `GLCanvas3D.cpp` 實作：
...
- [x] 新增 `void _on_prepare_clip_changed(double z_low, double z_high)`：
...
- [x] 在 `_render_imgui_overlay()`（或同等渲染函式）中加入對 `_render_prepare_clip_slider()` 的呼叫
...
- [x] 確認所有 Stage 1–4 的新程式碼均有 `ptSLA && CanvasView3D` guard
- [x] 確認 `m_prepare_clip_z_low/high/scene_max_z` 在 FDM 模式下不被修改
- [x] 確認 `_render_prepare_clip_slider()` 在 FDM 模式下不被呼叫

### 5.2 Preview 影響確認

- [x] 確認 `set_clipping_plane()` 呼叫都在 view3D canvas 上（非 preview canvas）
- [x] 確認 `on_sla_layer_slider_changed()` 邏輯未被修改
- [x] 確認 IMSlider（preview 的 m_layers_slider）未被修改
