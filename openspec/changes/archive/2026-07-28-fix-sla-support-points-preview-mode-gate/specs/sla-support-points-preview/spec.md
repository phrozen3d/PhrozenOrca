# sla-support-points-preview

## Overview

The SLA Support gizmo offers two preview view modes:

- **Points**: per-support-point preview cones (head + pillar shape) drawn from `m_normal_cache` (non-editing) or `m_editing_cache` (editing) inside `GLGizmoSlaSupports::render_points()`.
- **Structure**: the sliced support tree + pad mesh drawn from `m_volumes` inside `GLGizmoSlaBase::render_volumes()`.

The user-facing view-mode flag is `m_show_support_structure` (bool, default false). The Structure-mesh visibility flag is `m_show_sla_supports` (in `GLGizmoSlaBase`), driven through `show_sla_supports(bool)`.

`sla::SupportPoint::pos` is in raw model-object coordinates (the same space as the raycaster in `CommonGizmosDataObjects::Raycaster` and as the hit point returned by `GLGizmoSlaBase::unproject_on_mesh`). The visible mesh produced by `update_volumes()` is rendered with the full instance transform including scale.

This capability defines:

1. When the Points preview is allowed to render and intercept picking, in relation to the view-mode flag and the editing-mode flag (`m_editing_mode`).
2. How the Points-preview cone position, geometry size, and picking sphere relate to the instance transform — anchor follows scale, geometry stays in mm, picking matches the visible cone.
