...
- [x] 9.7 Structural mutation while a mode is open (delete focused object) — **confirmed passing after fix** (see 9b: found and fixed a crash — `BackgroundSlicingProcess::m_current_plate` going dangling when undo restores an older `PartPlateList` after the plate's only object is deleted). This was the scenario the original crash history (`e367bb0025`) that motivated this whole change was about; delete-all/plate-clear/load-project variants not separately re-tested after the fix, only delete-the-focused-object.
