...
### 9c. Gap found in the 9a/9b fix: `resync_after_undo_redo()` doesn't fire when landing on a state where no relevant gizmo is open

**Not yet fixed — root cause identified, scope assessment pending (see design.md Decision K).**
...
Root cause: `GLGizmosManager::update_after_undo_redo()` only calls a gizmo's `resync_after_undo_redo()` when that gizmo is `m_current` (the currently active one) *after* the undo/redo lands. In this redo, the restored snapshot's captured gizmo state is "no gizmo open" (since the user had already left Drill before this position was recorded) — so neither the Hollow nor the Drill branch fires, even though Decision H's backend-wipe-on-every-undo/redo still applies regardless of gizmo state. The result: correct model data, no trigger to recompute it, so the normal 3D view shows nothing for either.

This is a scope gap in the fix's *design*, not a one-off miss: **the underlying problem (Decision H) is scoped to "landed on a state that has hollow/drill/support data," but the fix (Decision I) is scoped to "landed on a state where the relevant gizmo happens to be currently open."** Those two scopes only partially overlap. This same gap plausibly also affects Support (9.5 was only tested with Support staying the active gizmo throughout — not re-tested for "redo/undo to a Support-applied state while a *different* gizmo, or no gizmo, ends up active").
