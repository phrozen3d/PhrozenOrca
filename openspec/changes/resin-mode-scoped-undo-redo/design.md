...
**Not fixed. Root cause identified; scope/risk assessment deferred pending user decision.**
...
**Why**: `GLGizmosManager::update_after_undo_redo()` calls a gizmo's `resync_after_undo_redo()` only when that gizmo is `m_current` *after* the undo/redo lands (see Decision I). This redo's target snapshot captured "no gizmo open" (the user had already left Drill by the time that position in history was recorded), so neither the Hollow nor the Drill branch fires — even though Decision H's finding (the backend cache is discarded on essentially *every* undo/redo, regardless of gizmo state) still applies here exactly as before. Net effect: correct data, nothing told to recompute it from that data, so the normal 3D view shows neither result.

**This is a scope mismatch between the problem and the fix, not a missed edge case**: Decision H's problem is scoped to *any state that has hollow/drill/support-relevant data*; Decision I's fix is scoped to *states where the relevant gizmo happens to still be open*. Those only partially overlap — every scenario tested and confirmed so far (9.5, 9b item 4) happened to land with the relevant gizmo still active. This same gap plausibly also affects Support, untested: undo/redo to a Support-applied state where Support isn't the gizmo left active.

**Two directions, not yet chosen between**:
- **Narrower**: make the resync decision model-data-driven instead of gizmo-active-driven — e.g. in `Plater::priv::update_after_undo_redo()` itself (not per-gizmo), unconditionally check the restored model for `hollowing_enable`/non-empty `sla_drain_holes`/`generate_support` and force the relevant `reslice_until_step()` calls regardless of which gizmo (if any) is currently open. Larger blast radius than Decision I (runs on *every* undo/redo, not just ones landing inside a resin gizmo) but stays within the same "explicit recompute trigger" shape, no new mechanism.
- **Broader**: address Decision H's actual root cause (`model.id() != m_model.id()` in `SLAPrint::apply()`) so the backend cache isn't discarded unnecessarily in the first place. Removes the need for any of the three `resync_after_undo_redo()` patches, but touches core undo/redo model-identity semantics with an unclear blast radius on whatever else depends on that check — assessed in Decision I as higher risk than the targeted patch, and that assessment still holds.
