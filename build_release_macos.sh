...
function pack_deps() {
    ...
    (
        set -x
        cd "$DEPS_DIR"
        tar -zcvf "PhrozenOrca_dep_mac_${ARCH}_$(date +"%Y%m%d").tar.gz" "build"
    )
}
...
function build_slicer() {
    ...
    for _ARCH in x86_64 arm64; do
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            if [ "1." != "$BUILD_ONLY". ]; then
                cmake "${PROJECT_DIR}" \
                    ...
                    -DCMAKE_INSTALL_PREFIX="$PWD/PhrozenOrca" \
                    -DCMAKE_BUILD_TYPE="$BUILD_CONFIG" \
        ...
        (
            ...
            rm -rf ./PhrozenOrca.app
            # fully copy newly built app
            cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/PhrozenOrca.app" ./PhrozenOrca.app
            # fix resources
            resources_path=$(readlink ./PhrozenOrca.app/Contents/Resources)
            rm ./PhrozenOrca.app/Contents/Resources
            cp -R "$resources_path" ./PhrozenOrca.app/Contents/Resources
            # delete .DS_Store file
            find ./PhrozenOrca.app/ -name '.DS_Store' -delete
        )

        # extract version
        # export ver=$(grep '^#define Phrozen_VERSION' ../src/libslic3r/libslic3r_version.h | cut -d ' ' -f3)
        # ver="_V${ver//\"}"
        # echo $PWD
        # if [ "1." != "$NIGHTLY_BUILD". ];
        # then
        ...
        # zip -FSr PhrozenOrca${ver}_Mac_${_ARCH}.zip PhrozenOrca.app
...
function build_universal() {
    ...
    mkdir -p "$PROJECT_BUILD_DIR/PhrozenOrca"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/PhrozenOrca/PhrozenOrca.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$PROJECT_DIR/build/arm64/PhrozenOrca/PhrozenOrca.app" "$UNIVERSAL_APP"
    
    # Get the binary path inside the .app bundle
    BINARY_PATH="Contents/MacOS/PhrozenOrca"
    
    # Create universal binary using lipo
    lipo -create \
        "$PROJECT_DIR/build/x86_64/PhrozenOrca/PhrozenOrca.app/$BINARY_PATH" \
        "$PROJECT_DIR/build/arm64/PhrozenOrca/PhrozenOrca.app/$BINARY_PATH" \
